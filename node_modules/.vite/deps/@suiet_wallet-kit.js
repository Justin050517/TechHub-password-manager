...
var _data;
var _client;
var ZkLoginPublicIdentifier = class extends PublicKey {
  /**
...
function sqrt9mod16(P) {
  ...
  return (Fp, n2) => {
    ...
    const e22 = Fp.eql(Fp.sqr(tv3), n2);
...
var wNAF = class {
  ...
  wNAF(W2, precomputes, n2) {
    ...
    let p = this.ZERO;
    let f = this.BASE;
    const wo2 = calcWOpts(W2, this.bits);
...
function isogenyMap(field, map) {
  ...
  return (x, y) => {
    const [xn, xd, yn2, yd] = coeff.map((val) => val.reduce((acc, i2) => field.add(field.mul(acc, x), i2)));
...
function schnorrSign(message, secretKey, auxRand = randomBytes(32)) {
  ...
  const t3 = Fn2.toBytes(d ^ num(taggedHash("BIP0340/aux", a3)));
  const rand = taggedHash("BIP0340/nonce", t3, px, m);
  const { bytes: rx, scalar: k } = schnorrGetExtPubKey(rand);
  const e3 = challenge(rx, px, m);
  const sig = new Uint8Array(64);
  sig.set(rx, 0);
  sig.set(Fn2.toBytes(Fn2.create(k + e3 * d)), 32);
...
function schnorrVerify(signature, message, publicKey) {
  ...
  try {
    ...
    if (!inRange(s, _1n5, secp256k1_CURVE.n))
      return false;
    const e3 = challenge(Fn2.toBytes(r), pointToBytes(P), m);
    const R = BASE.multiplyUnsafe(s).add(P.multiplyUnsafe(Fn2.neg(e3)));
...
// node_modules/@mysten/sui.js/dist/esm/multisig/signer.js
...
var MultiSigSigner = class extends Signer {
  constructor(pubkey, signers = []) {
    var _a2;
    super();
    __privateAdd3(this, _pubkey, void 0);
    __privateAdd3(this, _signers, void 0);
  ...
  async signTransactionBlock(bytes) {
    const signature = __privateGet3(this, _pubkey).combinePartialSignatures(
      await Promise.all(
        __privateGet3(this, _signers).map(async (signer) => (await signer.signTransactionBlock(bytes)).signature)
      )
    );
    return {
      signature,
...
// node_modules/@mysten/sui.js/dist/esm/multisig/publickey.js
...
// node_modules/@mysten/sui.js/dist/esm/verify/index.js
...
var rs = function() {
  function e3(n2) {
    this.options = U({}, n2.defaultOptions, n2.options), this.mutationId = n2.mutationId, this.mutationCache = n2.mutationCache, this.observers = [], this.state = n2.state || is(), this.meta = n2.meta;
  }
  var t3 = e3.prototype;
